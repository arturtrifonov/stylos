...
> **Superseded in two respects, 2026-08-28.** §4.1 forbids external fonts and §4.3 says "neutral and plain … do not hand-code Stylos colours into it". Both were aimed at the same failure, a page that reaches out or that carries a transcribed value, and both are kept — but the view is now dressed from `tokens/`. Colour, radius, the type scale and both families are resolved on every build by [`tools/lib/theme.mjs`](../../tools/lib/theme.mjs), which is the "natural first consumer" §4.3 was holding the slot for; nothing is hand-coded. The two families `font.yaml` names are **self-hosted** under `assets/fonts/` and referenced relatively, so nothing is fetched over the network and the test that enforces that is unchanged. The view is also no longer built alone: `npm run build` writes it into a publishable tree with the component pages and a placeholder home page. **§4.3's text search is withdrawn.** Every row is in the document — nothing is virtualised — so the browser's own find already searches the names, and every other column with them; a second, worse search box beside it was one more control to look past. Acceptance criterion 2 stands minus that word. It is still a working view of structured data and still not a documentation site.
