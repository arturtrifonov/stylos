...
Writes `build/registry.html` — the index over all entries: filter by level, role, readiness, wave and group, sort, follow relations as links. **Queue** is where [`PLAN.md`](../../../PLAN.md) puts the component — a numbered wave of the v0.1 core set (§4) or the group it is worked with afterwards (§9). It is read from the plan on every build and never stored on an entry, so filtering to a wave or a group is how you see what it is made of, and an entry the plan places nowhere is reported by the validator. It is what `import.batch` used to be here: that is Airtable's sequencing from the day of the import, history rather than the queue, so it is no longer a facet, a sort key or a column ([`0004`](../../specs/0004-registry-reconciliation.md) §3.4). Its values stay in the detail panel, under a heading naming their origin and date. Open it from disk; it reaches nothing over the network. The output is derived and gitignored, so rebuild it rather than looking for it in a checkout.
