...
Writes `build/registry.html` — the index over all entries: filter by level, role, readiness and Airtable batch, sort, follow relations as links. There is no search box — every row is in the document, so the browser's own find does it. Open it from disk; it reaches nothing over the network. The output is derived and gitignored, so rebuild it rather than looking for it in a checkout.
