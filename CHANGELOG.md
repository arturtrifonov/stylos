...
**Not done, and not startable here.** The Figma pass over the thirty-nine — the integrity and naming runs, the core renames, the `documentationLinks` targets, the `Meta` collection itself — is block B of the spec and is the whole variance in its estimate. `status` therefore stays `draft` on every entry, `[Unreleased]` stays unrolled, and no tag is cut. The variant-property rename behaviour is still untested in the Playground ([SPEC 0006](docs/specs/0006-versioning-and-release-0-1-0.md) §12).
