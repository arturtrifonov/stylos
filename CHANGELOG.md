...
- **[SPEC 0012](docs/specs/0012-icon-system.md)** specifies the icon set and the component that renders it. **It shrank twice while being written, both times by removing machinery**, and the spec is the result rather than the original: `<Icon name>` takes a Material Symbols name rather than a Stylos meaning slug, and there is no substitution layer at all.
