...
### Fixed — 2026-09-15 (the links the spec purge left behind)

- **Eighty-one links into `docs/specs/` and `docs/decisions/` no longer point at deleted files.** The directories went on 2026-09-10 and the citations did not. A retired spec or decision now keeps its number as bare text — `SPEC 0009`, `ADR 0002` — because the number is still a handle into git history and a link that 404s is not. Where something in the tree does carry what the citation was reaching for, the link points there instead: `README.md` and `tools/README.md` cite `ARCHITECTURE.md` §1 for the one-directional Figma rule, `CLAUDE.md` cites it for the frontend stack, and `docs/components/registry/README.md` cites `tools/README.md` for the page generator.
- **Four documents stopped claiming directories that do not exist.** `ARCHITECTURE.md` listed `docs/decisions/` as a source of truth in §1 and again in the §6 hierarchy; architectural decisions are recorded in `ARCHITECTURE.md` itself, and its row now says so. `docs/README.md` listed `specs/`, `decisions/` and `research/` — the third has never existed in this tree. `PLAN.md` said things to be built get a work order in `docs/specs/`; they are sequenced in `PLAN.md` §4 and §9, and nothing replaces the work orders.
- **`ARCHITECTURE.md` §5 records what the purge cost.** Most citations were provenance and lose nothing by being unlinked. Four are load-bearing and now point at content that exists only in git history: the props ↔ `api` mapping rule behind the `0.2.0` gate (SPEC 0009 §3), the independent CSS export (SPEC 0010 §2.2), the emitted icon shape (SPEC 0012) and ADR 0002's revisit clause, which `PLAN.md`'s risk table leans on. Named rather than papered over.
- `docs/foundations/color.md` and `docs/foundations/effects.md` are not in this change — their spec links are fixed in the colour branch, which is already rewriting both files.
...
- Work order: SPEC 0013, migration step 1 of three.
...
- **§"Two audiences, and they are not the same size" is removed.** SPEC 0010 §9 still owes a paragraph naming a coding agent reading `registry.json` as a third audience; the amendment there records that the section it was to be added to is gone, and leaves where it goes to the pull request that lands Part B.
...
- **`name` takes a Material Symbols name**, decided by the owner in this session per `CLAUDE.md`. The contract's `api.name` moves from `variant` (twenty-nine title-case values) to `text`, amended in place and dated.
- **The entry's prose stops describing Figma's indirection as though it were the code contract.** `summary`, `purpose`, `use_when`, the second `do_not_use_when` and the first `figma_note` all carried the meaning-proxy framing — a mark "chosen by what it means rather than by how it is drawn", a drawing "replaced without any consumer changing", adding one being "a decision about the vocabulary". None of that describes what code does: a name is written, and adding a mark is a line in `assets/icons/manifest.yaml` and a re-run.
- **No default and no placeholder.** A name the build does not carry renders nothing. A default would mean a forgotten prop silently drawing a real mark, which is worse than drawing none: nothing is visibly wrong, a magnifier is invisibly wrong.
- **The set grows to the twenty-nine the Figma library already ships** as system components, read from node `2839:2469` — demand-backed rather than anticipated.
- **`tools/build-ui-icons.mjs`** projects the committed SVGs into `icons.ts` as data, so nothing parses markup at runtime and nothing needs `{@html}`. The shape it emits is the one a client's replacement set must satisfy, so the default set and a replacement are one type.

### Changed — 2026-09-07 (a text property's values are examples)
...
- **`chevron_right, check_circle, more_horiz` displayed as "chevronright, checkcircle, morehoriz"** on Storybook's docs page, with everything after them italicised. `build-ui-stories.mjs` assembles the contract's prose into one Markdown block, and a bare underscored identifier is read as emphasis — so the one component whose every value is underscored documented its values wrong. Icon's description is rewritten without example names in the prose; the examples live in `values`, which is not rendered as Markdown and shows them intact.
- The rule is recorded in [the registry README](docs/components/registry/README.md) rather than only fixed here, because the next contract naming an identifier in prose will hit the same thing.

### Fixed — 2026-09-07 (the footprint was not adjustable)

- The icon declared its footprint custom property **on its own rule**, where the element's own declaration beats the one it inherits — so a parent setting the property moved nothing, against the contract's `adjustable: true`. The token is now the `var()` fallback instead of a declaration, and a parent with a size run of its own sets `--stylos-icon-footprint` and the icon follows. Caught by measuring it in the workshop rather than by reading the rule.

### Added — 2026-09-07 (SPEC 0012 — the icon system)

- **SPEC 0012** specifies the icon set and the component that renders it. **It shrank twice while being written, both times by removing machinery**, and the spec is the result rather than the original: `<Icon name>` takes a Material Symbols name rather than a Stylos meaning slug, and there is no substitution layer at all.
- **What was removed, and why it was wrong to propose.** The first draft translated Figma's thirty system components into code as a runtime provider, a replaceable `IconSet` and a client-side set generator. Figma's indirection exists because a designer picks visually out of a connected library; a developer does not browse, they write the name of the mark they want, and if the set lacks it the fix is a line in the manifest — not a mechanism. Translating it would have bought replaceability nobody asked for and charged every component for it, each resolving its marks through a context instead of naming them.
- **The twenty-nine marks are recorded with their drawings**, read from Figma node `2839:2469` and mapped to Material Symbols names verified against the pinned package. The mapping was checked against the drawings rather than inferred from the words, which caught `File` being a paperclip (`attach_file`), `More` being horizontal (`more_horiz`), and `Expand More`/`Expand Less` not existing under those names in Symbols at all (`keyboard_arrow_down`/`keyboard_arrow_up`).
- **The whole set ships, and stays small on purpose.** Twenty-nine drawings inlined into `@stylos/ui`, about 32 KB, as data rather than markup (so nothing needs `{@html}`) and rather than a stylesheet (so a drawing is addressable by name). Shipping all of it is affordable only because a mark enters the manifest when something needs it; the two rules hold each other up.
...
- **`Icon` is built** — the first entry of PLAN.md's second wave. The element is the `<svg>` itself, which is what the contract's `html` says, so the footprint is the element and there is no wrapper to size separately. Always `aria-hidden="true"` and `focusable="false"`: whether the mark needs a name is the parent's question and nothing in the component can answer it, so the one thing it refuses to let a call site re-decide is the thing 68 registry entries would otherwise each have to remember.
- **`name` takes a Material Symbols name**, decided by the owner in this session per `CLAUDE.md`. One door over the whole library beat a Stylos vocabulary covering twenty-nine of 3,905. The contract's `api.name` moves from `variant` (twenty-nine title-case values) to `text`, amended in place and dated; SPEC 0012 carries the same amendment, including what it costs — the contract's "not entitled to the shape behind it" is spent, because the consumer now names a drawing.
- **No default and no placeholder.** A name the build does not carry renders nothing. A default would mean a forgotten prop silently drawing a real mark, which is worse than drawing none: nothing is visibly wrong, a magnifier is invisibly wrong.
- **The set grows to the twenty-nine the Figma library already ships** as system components, read from node `2839:2469` — demand-backed rather than anticipated.
- **`tools/build-ui-icons.mjs`** projects the committed SVGs into `icons.ts` as data, so nothing parses markup at runtime and nothing needs `{@html}`. The shape it emits is the one a client's replacement set must satisfy, so the default set and a replacement are one type.
...
- **SPEC 0012** specifies how icons reach a consuming project and how a consumer replaces them. It reads the Figma model as the design rather than as a Figma detail: the thirty system components are a **meaning layer**, and their two jobs — one replacement reaching every component, and the whole published library staying available — are the requirements the code has to meet.
- **The twenty-nine meanings are recorded with their drawings**, read from Figma node `2839:2469` and mapped to Material Symbols names verified against the pinned package. The mapping was checked against the drawings rather than inferred from the words, which is what caught `File` being a paperclip (`attach_file`), `More` being horizontal (`more_horiz`), and `Expand More`/`Expand Less` not existing under those names in Symbols at all (`keyboard_arrow_down`/`keyboard_arrow_up`).
- **The full library stays reachable, and the instance does not open.** Drawings ship as data rather than as a stylesheet, because a stylesheet cannot be tree-shaken and the full set is ~4.8 MB: `@stylos/ui` inlines the 29 defaults (~32 KB, no dependency on the full package), `@stylos/icons` carries all 3,905 with an `index.json` that is the agent's list. A client replaces the set through a `<StylosIcons>` provider taking a partial set, and generates their own with a converter that needs no font — so `fontkit`, `wawoff2` and `material-symbols` never leave this repository.
- **The placeholder does not cross into code**: an icon that was not chosen is not written, and an unresolved meaning renders nothing. `Success`, `Warning` and `Error` become filled in the swap, named in [`icons.md`](docs/foundations/icons.md) so the change is not read as a defect.

### Changed — 2026-09-07 (icons stop being interim)

- [`docs/foundations/icons.md`](docs/foundations/icons.md) no longer describes the icon source as a link to someone else's Figma library. It now records the decision, made by the owner in an implementation session per `CLAUDE.md`: **Material Symbols as the drawing source, SVG as the delivery form, one instance rather than live axes.** The reasoning for refusing an icon font is written down as the failure class it is — font blocked, fonts overridden by the reader, page machine-translated — rather than as the screen-reader argument, which both forms answer with the same two lines. The accessibility contract (`aria-hidden` on the mark, the name on the control) is stated once for every icon regardless of what renders it. The superseded Material Icons section is kept, marked, with the one thing it got right: this is still a placeholder for a native Stylos set, not a claim that Material matches the system's character.
- **A consequence recorded rather than discovered later:** shipping `FILL 1` only spends the FILL axis, which in Material Symbols is the unselected → selected pair. Components that want to show selection cannot reach for a filled/unfilled pairing from this set.
- The Figma libraries still point at Default Kit / Material Icons, so a component's Figma face and its code face are currently different drawings. Named as open in `icons.md`, cheapest to fix before the Button family fills its icon slots.
...
Built to SPEC 0009. The repository is now an npm workspace: `packages/ui` is the Svelte 5 component package, `apps/workshop` is its Storybook.
...
Built to SPEC 0008. On 2026-09-05 an agent's command fast-forward-merged a branch into `master` because there was nothing to refuse it; this is the flow that refuses it. **No component, token or contract value changed.**
...
- SPEC 0003 carries the three fields in §3.1, §3.2 and §4.1, in the same change that added them.
...
- **Still derived, never authored** (SPEC 0002 §3.2). `derive()` keeps its shape and readiness is composed from it, so there is no third fact to keep in step and nothing to tick by hand.
...
Built to SPEC 0003. The contract became data on 2026-08-26; this is what reads it.
...
- Decision record: the Figma connection model (decision 0001).
