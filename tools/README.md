...
`build-css.mjs` projects the canonical set onto CSS custom properties — 892 of them, into `packages/ui/dist/tokens.css`, with `tokens.json` recording which canonical token each name came from. Built to SPEC 0007. Neither file is committed. It reads `tokens/*.yaml` and nothing else, and it refuses to run at all on a set that fails `tokens:check`.

Two things in it are authored rather than read, because Figma has no field for them: the font fallback stacks and the shadow composition. One claim is checked rather than authored — a role with a `special` segment names the hue group it must land in, so `surface/special/violet` bound to indigo fails the build naming both (FND-COLOR-08). Every other role is named for what it paints and may be bound to any step, so there is nothing about it to check.

*Until 2026-09-15 this stage also synthesised a layer of `--stylos-slot-*` properties, five names × thirteen steps, and enforced that every non-`special` role reached one of five hue groups. Both are gone: no rule of the design language says a role belongs to one of five groups, nothing consumed the properties, and the check failed a colour decision that broke nothing.*
