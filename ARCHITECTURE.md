...
**This document is normative.** It describes the system as it actually is on the date below — not as it is intended to become. Anything that does not exist is listed as not existing. History belongs in [`CHANGELOG.md`](CHANGELOG.md) and in git; a rule's rationale belongs beside the rule, in the `Why:` its block carries; rules of the design language belong in [`docs/foundations/`](docs/foundations/README.md).
...
**A rule is a block with an ID**, in the grammar [`docs/RULES.md`](docs/RULES.md) fixes, so that a contract, a skill or a test can cite one and `npm run validate:rules` can fail a citation that points at nothing. Most of that set is empty and says so: a file marked *Yet to fill* is a place for a rule, not a claim that one exists (SPEC 0013).
...
- **Rules go in the four guideline directories** — `docs/foundations/`, `docs/behavior/`, `docs/patterns/`, `docs/content/` — one file per topic, each rule with an ID and its reasoning beside it, in the form [`docs/RULES.md`](docs/RULES.md) fixes. Everything else is a rule or a `CHANGELOG.md` line: `docs/decisions/` and `docs/specs/` were retired on 2026-09-10, and a decision that is not a rule is now recorded where it lands — in this document, in the guideline it changes, or in the release notes (§5).
...
- **A CSS or platform feature is relied on once [Baseline](https://web.dev/baseline) records it as widely available** — interoperable across the core browser set, and stable there for the window Baseline defines. Deliberately not an authored browser list: Baseline is checkable mechanically, because `web-features` data answers "is X widely available" with a lookup rather than a judgement call; it is maintained by someone else, where a list written here ("last 2 versions of…") would be stale within a quarter and re-evaluated by nobody; and it is stricter than the scope strictly needs, since Baseline's browser set includes mobile and Stylos is desktop-only. That last one is accepted rather than corrected — a feature held back solely by a browser outside the scope is what the exception is for. **A feature below the floor may be used where the code says which feature, why, and what happens in a browser without it** — a `@supports` fallback, or a degradation that loses polish rather than function. An exception nobody wrote down is a bug. This is a constraint on the build and not a rule of the design language, which is why it carries no rule ID: that a browser supports a feature is never a reason for a design to use it, and the floor's only power over a design is a veto on what cannot be built.
